...
├─ providers.tf      provider/버전, 원격 상태(S3 백엔드)
...
### 3) 인프라 자동화 (GitOps · 실무 표준) ✨

손으로 `terraform apply` 하던 것을 **GitHub Actions(`infra.yml`)** 가 대신합니다.

```
PR 에서 infra/terraform/** 수정
        │  terraform plan
        ▼
  PR 코멘트로 plan 결과 게시  ← 사람이 리뷰
        │  approve & merge
        ▼
  main push → terraform apply 자동 실행
```

- **인증:** OIDC 키리스. 앱 배포(`deploy.yml`)와 분리된 **전용 역할 `pawtrace-gha-terraform`** 을 사용(직무 분리).
- **상태:** S3(`pawtrace-tfstate-*`) + DynamoDB 잠금으로 원격 공유.

**최초 1회 설정:**
```bash
# (1) bootstrap 재적용 → terraform 전용 역할 생성 (terraform_role_arn 출력)
cd infra/terraform/bootstrap && terraform apply

# (2) 역할 ARN 을 Secret 으로 등록
gh secret set AWS_TF_ROLE_ARN -R ggaeun324-wq/pawtrace --body "<terraform_role_arn>"
```
이후에는 PR → 머지만으로 인프라가 배포됩니다.
...
사용 후 `terraform destroy` 로 정리하세요. IaC 라 **필요할 때 5분이면 동일하게 재생성**됩니다.
...
<!-- pipeline smoke test: plan-on-PR 동작 확인 -->
